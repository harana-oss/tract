...
fn bench_argmax(c: &mut Criterion) {
    ...
    for cols in [8usize, 32, 128, 512, 2048] {
        ...
        group.bench_with_input(BenchmarkId::new("argmax_row", cols), &cols, |b, &cols| {
            b.iter(|| {
                let idx = tract_onnx_opl::ml::argmax::argmax_row(black_box(&row));
                black_box(idx)
            })
        });
    }
    ...
    for (rows, cols) in [(128usize, 32usize), (64, 128), (32, 512), (16, 2048)] {
        ...
        group.bench_with_input(BenchmarkId::new("argmax_rows", format!("{}x{}", rows, cols)), &(rows, cols), |b, &(rows, cols)| {
            b.iter(|| {
                let out = tract_onnx_opl::ml::argmax::argmax_rows(black_box(&data), rows, cols);
                black_box(out)
            })
        });
        ...
        for r in 0..rows {
            data_g[r * stride..r * stride + cols]
                .copy_from_slice(&data[r * cols..(r + 1) * cols]);
        }
        group.bench_with_input(BenchmarkId::new("argmax_rows_gather", format!("{}x{}(s0={},s1=1)", rows, cols, stride)), &(rows, cols), |b, &(rows, _cols)| {
            b.iter(|| unsafe {
                let out = tract_onnx_opl::ml::argmax::argmax_rows_gather(
                    data_g.as_ptr(), rows, cols, stride as isize, 1,
                );
